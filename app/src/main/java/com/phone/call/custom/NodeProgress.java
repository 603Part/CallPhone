...
public class NodeProgress extends View {
    ...
    public NodeProgress(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mDotsCount = typedArray.getInt(R.styleable.NodeProgress_barDotsCount, 2);
        mDotsRadius = typedArray.getDimensionPixelSize(R.styleable.NodeProgress_barDotsRadius, dp2px(8));
        mDotsProgressWidth = typedArray.getDimensionPixelSize(R.styleable.NodeProgress_barProgressWidth, dp2px(8));
        if ((2 * mDotsRadius) < mDotsProgressWidth)
            mDotsProgressWidth = mDotsRadius * 2; // 如果用户设置进度条的宽度大于点的直径，则设置为半径大小
        mDotsProgressWidthHalf = mDotsProgressWidth / 8;
        // 获取用户定义的时间，并转化成时间间隔
        mSpeed = typedArray.getInt(R.styleable.NodeProgress_barSpeed, 40);
        // 获取用户定义的进度条背景色与前景色
        mDotsBackColor = typedArray.getColor(R.styleable.NodeProgress_barBackColor, ContextCompat.getColor(context, R.color.gray));
        mDotsFrontColor = typedArray.getColor(R.styleable.NodeProgress_barFrontColor, ContextCompat.getColor(context, android.R.color.white));
        typedArray.recycle();
        // 初始化画笔
        mPaint = new Paint();
